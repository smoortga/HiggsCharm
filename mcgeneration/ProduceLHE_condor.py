...
f_tmp_btach_.write("export X509_USER_PROXY=/user/$USER/x509up_u$(id -u $USER) \n")
f_tmp_btach_.write("tar -xaf $1 \n")
f_tmp_btach_.write("sed -i 's/exit 0//g' runcmsgrid.sh \n")
f_tmp_btach_.write("source runcmsgrid.sh $2 $3 1 \n")
#f_tmp_btach_.write("gfal-copy -f file://$TMPDIR/cmsgrid_final.lhe srm://maite.iihe.ac.be:8443%s/cmsgrid_final_$4.lhe"%(args.outdir))
f_tmp_btach_.write("cp $TMPDIR/cmsgrid_final.lhe %s/cmsgrid_final_$4.lhe"%(args.outdir))
f_tmp_btach_.close()
...
f_tmp_condor_.write(" \n")
f_tmp_condor_.write("Error = condor_log_%s/job_$(number).err \n"%((args.tag).replace(" ","_")))
f_tmp_condor_.write("Output = condor_log_%s/job_$(number).out \n"%((args.tag).replace(" ","_")))
f_tmp_condor_.write("Log = condor_log_%s/job_$(number).log \n"%((args.tag).replace(" ","_")))
f_tmp_condor_.write(" \n")
