...
--indir=/pnfs/iihe/cms/store/user/smoortga/HiggsCharm/RECOAODMINIAOD_testing_MINIAOD/ \
--outdir=/pnfs/iihe/cms/store/user/smoortga/HiggsCharm/NANOAOD \
--jobflavour=tomorrow \
--tag=testing_NANOAOD
...
for f_ in files_:
	...
	ff_.write("cmsRun %s infile=file:%s%s outfile=file:$TMPDIR/%s_NANOAOD_%i.root nevents=-1 \n"%(config,indir,f_,args.tag,number_))  
	ff_.write("cp $TMPDIR/%s_NANOAOD_%i.root %s%s_NANOAOD_%i.root \n"%(args.tag,number_,outdir,args.tag,number_))
	ff_.write("rm $TMPDIR/%s_NANOAOD_%i.root \n"%(args.tag,number_))
	ff_.close()
