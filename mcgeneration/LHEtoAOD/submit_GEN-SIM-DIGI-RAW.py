...
--indir=/pnfs/iihe/cms/store/user/smoortga/HiggsCharm/Higgs_Effective_Couplings_Test/ \
--outdir=/pnfs/iihe/cms/store/user/smoortga/HiggsCharm/GENSIMDIGI/ \
--jobflavour=tomorrow \
--tag=testing
...
files_ = [i for i in os.listdir(indir) if i.endswith(".lhe")]
for f_ in files_:
...
for f_ in files_:
	...
	ff_.write("cmsRun %s infile=file:%s%s outfile=file:$TMPDIR/%s_GEN-SIM-DIGI-RAW_%i.root nevents=-1 \n"%(config,indir,f_,args.tag,number_))  
	ff_.write("cp $TMPDIR/%s_GEN-SIM-DIGI-RAW_%i.root %s%s_GEN-SIM-DIGI-RAW_%i.root \n"%(args.tag,number_,outdir,args.tag,number_))
	ff_.write("rm $TMPDIR/%s_GEN-SIM-DIGI-RAW_%i.root \n"%(args.tag,number_))
	ff_.close()
